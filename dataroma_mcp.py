...
def main():
    load_cache()
    # CRITICAL FIX for MCP Protocol (stdio transport):
    # Hermes and Claude Desktop communicate with this server by reading standard output (stdout).
    # If fastmcp prints its visual ASCII banner to stdout, the client will fail to parse the JSON and close the connection.
    # Therefore, we MUST explicitly disable the banner and force the transport to "stdio".
